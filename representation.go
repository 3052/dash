...
func (r *Representation) requiresOriginalID() bool {
   ...
   // If the Media template uses the $RepresentationID$ variable, we SHOULD NOT rename it.
   // We ignore Initialization here, assuming the Media template is the
   // authoritative source of structural dependency on the ID.
...
func (r *Representation) getContinuityPattern() string {
   st := r.GetSegmentTemplate()
   if st != nil && st.Media != "" {
      // We rely on the Media template as the unique signature.
      return st.Media
   }
   // Fallback for SegmentList/BaseURL (or empty Media template):
   // use the original ID as the pattern before it gets overwritten.
   return r.ID
}
